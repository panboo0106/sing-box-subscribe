...
def get_nodes(url):
    ...
    if type(content) == dict:
        if "proxies" in content:
            ...
            filtered_outbounds = [
                outbound for outbound in content["outbounds"] if outbound.get("type") not in excluded_types
            ]
        ...
        if rule["outbound"] not in ["block", "dns-out"]:
            if rule["outbound"] != "direct":
                outbounds_dns_template = list(
                    filter(lambda server: server["tag"] == asod["proxy"], config["dns"]["servers"])
                )[0]
            ...
            if "subgroup" not in group:
                for out in config_outbounds:
                    if out.get("outbounds"):
                        if out["tag"] == "Proxy":
                            out["outbounds"] = (
                                [out["outbounds"]] if isinstance(out["outbounds"], str) else out["outbounds"]
                            )
